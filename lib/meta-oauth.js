...
export function getFacebookConfiguration() {
  const missing = FACEBOOK_REQUIRED_ENV.filter((name) => !process.env[name]);
  return { ready: missing.length === 0, missing };
}

export function getFacebookRedirectUri() {
  return process.env.FACEBOOK_REDIRECT_URI
    || "https://horeca-os-iota.vercel.app/api/integrations/facebook/callback";
}

export function getSafeReturnOrigin(origin) {
  try {
    const url = new URL(origin);
    if (url.protocol === "https:" && (url.hostname === "horeca-os-le-club.vercel.app" || url.hostname === "horeca-os-iota.vercel.app")) {
      return url.origin;
    }
  } catch {}
  return "https://horeca-os-le-club.vercel.app";
}
