...
export function instagramEventMedia(item, linkedSources = []) {
  ...
  function add(value, type, label) {
    ...
    seen.add(asset.url);
    const convertible = detectedType === "image" && (["image/png", "image/webp"].includes(mime) || /\.(png|webp)(\?|$)/i.test(asset.url));
    const unsupportedImage = detectedType === "image" && !convertible && ((mime && mime !== "image/jpeg") || /\.(gif|svg)(\?|$)/i.test(asset.url));
    result.push({ ...asset, label: entry.label || entry.name || label, width: Number(entry.width) || null, height: Number(entry.height) || null,
      needsJpeg: convertible, issue: unsupportedImage ? "Gebruik een JPG-, PNG- of WebP-foto; dit bestandstype kan niet worden omgezet." : "" });
  }
