...
export default function Home() {
  ...
  const total30 = data.sales.reduce((sum, row) => sum + number(row.revenue_inc_vat), 0);
  const orders30 = data.sales.reduce((sum, row) => sum + number(row.order_count), 0);
  const averageTicket = orders30 ? total30 / orders30 : number(latestSales?.avg_order_value);
  ...
  const takeawayRevenue = number(latestSales?.takeaway_revenue);
  const ownChannelRevenue = number(latestSales?.own_channel_revenue);
  const latestRevenue = number(latestSales?.revenue_inc_vat);

  const takeawayShare = latestRevenue
    ? (takeawayRevenue / latestRevenue) * 100
    : 0;

  const ownChannelShare = latestRevenue
    ? (ownChannelRevenue / latestRevenue) * 100
    : 0;
    ...
    <div className="shell">
      ...
      <main className="content">
        ...
        <section className="kpis">
          <Card label="Laatste dagomzet" value={money(latestSales?.revenue_inc_vat)} sub={latestSales?.sales_date ? formatShortDate(latestSales.sales_date) : "Nog geen omzetdata"} />
        ...
        <section className="panel">
          <div className="panelHead">
            <div>
              ...
              <p>Verdeling van de laatst beschikbare verkoopdag</p>
            </div>
          </div>
          ...
          <div className="channelGrid">
            <ChannelCard
              ...
              orders={latestSales?.takeaway_order_count}
              share={takeawayShare}
            />

            <ChannelCard
              label="Eigen website en app"
              revenue={ownChannelRevenue}
              orders={latestSales?.own_channel_order_count}
              share={ownChannelShare}
            />

            <ChannelCard
              label="Bezorgorders"
              revenue={latestSales?.delivery_fees_inc_vat}
              orders={latestSales?.delivery_order_count}
              share={0}
              revenueLabel="Bezorgkosten"
            />

            <ChannelCard
              label="Afhaalorders"
              revenue={0}
              orders={latestSales?.pickup_order_count}
              share={0}
              revenueLabel="Aantal geregistreerd"
            />
          </div>
        </section>
  ...
  return (
    <article className="channelCard">
      ...
      <div className="channelMeta">
        <small>{number(orders)} orders</small>
        <small>
          {share > 0
            ? `${share.toFixed(1)}% van dagomzet`
            : revenueLabel}
        </small>
      </div>
    </article>
