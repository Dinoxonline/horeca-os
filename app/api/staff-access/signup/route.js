...
async function sendApprovalEmail({ email, fullName }) {
  ...
  const response = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      htmlContent: `<p>Er is een nieuwe accountaanvraag voor Horeca OS.</p><p><strong>Naam:</strong> ${escapeHtml(fullName)}<br /><strong>E-mail:</strong> ${escapeHtml(email)}</p><p><a href="https://horeca-os-le-club.vercel.app/gebruikers">Open Horeca OS → Gebruikers & rollen</a> om de aanvraag goed te keuren of af te wijzen.</p>`,
    }),
  });
