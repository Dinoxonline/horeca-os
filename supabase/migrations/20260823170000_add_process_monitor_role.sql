...
begin
  for w in select id as workspace_id from public.workspaces loop
    ...
    if role_id is null then
      select id into role_id from public.roles where workspace_id = w.workspace_id and role_key = 'shift_lead';
    end if;

    insert into public.role_permissions (workspace_id, role_id, permission)
    values
      (w.workspace_id, role_id, 'processes:read'),
      (w.workspace_id, role_id, 'processes:monitor')
