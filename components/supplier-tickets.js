...
export default function SupplierTickets({ workspaceId, suppliers = [], canManage = false }) {
  ...
  async function createTicket(event) {
    ...
    const { data: ticket, error } = await supabase.from("staff_tickets").insert({ workspace_id: workspaceId, ticket_kind: "supplier", supplier_id: supplier.id, supplier_email: draft.supplier_email.trim() || supplier.support_email || null, supplier_contact: draft.supplier_contact.trim() || supplier.contact_name || null, category: "Leveranciersupport", priority: draft.priority, status: "nieuw", title: draft.title.trim(), description: draft.description.trim(), location: null, reporter_name: "Horeca OS", reporter_contact: draft.supplier_email.trim() || supplier.support_email || null }).select("id").single();
    setSaving(false);
