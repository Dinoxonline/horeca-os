...
const emailTemplates = { standaard: { label: "Standaard supportvraag", intro: "Wij melden hierbij een probleem waarvoor wij graag jullie hulp ontvangen." }, urgent: { label: "Urgente storing", intro: "Dit betreft een urgente storing die onze werkzaamheden direct beïnvloedt." }, aanvulling: { label: "Aanvulling op bestaand ticket", intro: "Hierbij geven wij een aanvulling op de eerder gemelde supportvraag." } };
...
export default function SupplierTickets({ workspaceId, suppliers = [], canManage = false }) {
  ...
  const [emailTemplatesByTicket, setEmailTemplatesByTicket] = useState({});
  ...
  async function sendSupplierEmail(ticket) {
    ...
    const template = emailTemplates[emailTemplatesByTicket[ticket.id] || "standaard"] || emailTemplates.standaard;
    const content = `${template.intro}\n\nTicket #${ticket.ticket_number}\n\n${ticket.description}\n\nVermeld in je antwoord ticketnummer #${ticket.ticket_number}, zodat wij alles aan hetzelfde dossier kunnen koppelen.`;
    const attachments = [];
  ...
  function previewFor(ticket) {
    const template = emailTemplates[emailTemplatesByTicket[ticket.id] || "standaard"] || emailTemplates.standaard;
    return { subject: `[Horeca OS #${ticket.ticket_number}] ${ticket.title}`, body: `${template.intro}\n\nTicket #${ticket.ticket_number}\n\n${ticket.description}\n\nVermeld in je antwoord ticketnummer #${ticket.ticket_number}, zodat wij alles aan hetzelfde dossier kunnen koppelen.` };
  }
