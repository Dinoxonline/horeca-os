...
function ReviewsInbox({ workspaceId, businessId, businesses, canManage, canAdd }) {
  const [reviews, setReviews] = useState([]); const [reviewMessage, setReviewMessage] = useState(""); const [statusFilter, setStatusFilter] = useState("all"); const [reviewPage, setReviewPage] = useState(1);
  const loadReviews = useCallback(async () => { let query = supabase.from("customer_reviews").select("*").eq("workspace_id", workspaceId).order("reviewed_at", { ascending: false }); if (businessId !== "all") query = query.eq("business_id", businessId); const { data: rows, error } = await query; if (error) setReviewMessage(`Reviews konden niet worden geladen: ${error.message}`); else setReviews(rows || []); }, [businessId, workspaceId]);
