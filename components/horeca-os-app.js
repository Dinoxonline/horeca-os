  ...
  return (
    <div className="shell">
      ...
      <main className="content">
        ...
        {activeView === "suppliers" && featureVisibility.suppliers && <SupplierOverview workspaceId={workspaceId} suppliers={data.suppliers} products={data.foodProducts} canManage={isOwner || canUseFeature("foodcost:manage")} onRefresh={loadData} />}
    ...
    <div className="stackList">
      {campaigns.map((campaign) => {
        ...
        const campaignSummary = cleanCampaignText && cleanCampaignText !== campaignHeading
          ? `${cleanCampaignText.slice(0, 240)}${cleanCampaignText.length > 240 ? "…" : ""}`
          : "";
        const labels = {
...
function ReviewsInbox({ workspaceId, businessId, businesses, session, canManage, canAdd }) {
  ...
  async function uploadReviews(event) {
    ...
    setReviewMessage(error ? `Upload niet verwerkt: ${error.message}` : `${payload.length} reviews succesvol geïmporteerd.`); if (!error) { event.currentTarget.reset(); loadReviews(); }
  }
...
function FoodcostDashboard({ analytics }) {
  return <>
    ...
    <section className="kpis">
      <Card label="Gemiddelde foodcost" value={analytics.average == null ? "—" : `${analytics.average.toFixed(1)}%`} sub={`${analytics.items.length} verkoopbare gerechten`} />
      <Card label="Beste marge" value={analytics.best?.name || "—"} sub={analytics.best ? `${analytics.best.foodcost.toFixed(1)}% foodcost` : "Nog geen complete kostprijs"} tone="success" />
      <Card label="Hoogste foodcost" value={analytics.worst?.name || "—"} sub={analytics.worst ? `${analytics.worst.foodcost.toFixed(1)}% foodcost` : "Nog geen complete kostprijs"} tone={analytics.worst?.foodcost > 40 ? "danger" : "normal"} />
    ...
    <section className="panel"><div className="panelHead"><h2>Gerechten</h2><p>Foodcost is exclusief btw-effecten en volgt de actuele productprijs.</p></div>
      <div className="tableWrap"><table><thead><tr><th>Gerecht</th><th>Kostprijs</th><th>Verkoopprijs</th><th>Foodcost</th><th>Doel</th><th>Status</th></tr></thead><tbody>
        {analytics.items.map((item) => <tr key={item.id}><td><b>{item.name}</b></td><td>{money(item.cost)}</td><td>{money(item.sellingPrice)}</td><td>{item.foodcost.toFixed(1)}%</td><td>{item.target ? `${item.target}%` : "—"}</td><td><span className={`status ${item.warning ? "pending" : "connected"}`}>{item.warning ? "Controleren" : "Op koers"}</span></td></tr>)}
      </tbody></table></div>{!analytics.items.length && <Empty text="Voeg producten, ingrediënten, receptregels en menu-items toe om foodcost te berekenen." />}
    </section>
  </>;
}
...
function SupplierOverview({ workspaceId, suppliers, products, canManage, onRefresh }) {
  ...
  async function createSupplier(event) {
    ...
    const { error } = await supabase.from("suppliers").insert({ workspace_id: workspaceId, name: form.name.trim(), contact_name: form.contact_name.trim() || null, support_email: form.support_email.trim() || null, support_phone: form.support_phone.trim() || null, active: true });
    if (error) { setMessage("De leverancier kon niet worden aangemaakt."); return; }
    setForm({ name: "", contact_name: "", support_email: "", support_phone: "" }); setOpen(false); setMessage("Leverancier aangemaakt."); onRefresh();
  }
  async function saveSupplier(event, supplierId) {
    event.preventDefault(); setSavingId(supplierId); setMessage("");
    const values = editing[supplierId];
    const { error } = await supabase.from("suppliers").update({ name: values.name.trim(), contact_name: values.contact_name.trim() || null, email: values.email.trim() || null, phone: values.phone.trim() || null, support_email: values.support_email.trim() || null, support_phone: values.support_phone.trim() || null, support_notes: values.support_notes.trim() || null }).eq("id", supplierId).eq("workspace_id", workspaceId);
    setSavingId("");
    if (error) { setMessage("De leveranciersgegevens konden niet worden opgeslagen."); return; }
    setEditing((current) => { const next = { ...current }; delete next[supplierId]; return next; }); setMessage("Leveranciersgegevens opgeslagen."); onRefresh();
  }
  function beginEdit(supplier) { setEditing((current) => ({ ...current, [supplier.id]: { name: supplier.name || "", contact_name: supplier.contact_name || "", email: supplier.email || "", phone: supplier.phone || "", support_email: supplier.support_email || "", support_phone: supplier.support_phone || "", support_notes: supplier.support_notes || "" } })); }
  return <DataPage title="Leveranciers" subtitle="Leveranciers, contactpersonen en gekoppelde inkoopproducten"><div className="panelHead"><div><p>Vul per leverancier de contactgegevens en het supportadres in. Deze gegevens worden vooraf ingevuld bij nieuwe leverancierstickets.</p></div>{canManage && <button type="button" className="primary" onClick={() => setOpen((value) => !value)}>{open ? "Formulier sluiten" : "Nieuwe leverancier"}</button>}</div>{message && <div className="notice">{message}</div>}{open && <form className="supplierTicketForm" onSubmit={createSupplier}><label>Naam leverancier *<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Contactpersoon<input value={form.contact_name} onChange={(event) => setForm({ ...form, contact_name: event.target.value })} placeholder="Naam contactpersoon" /></label><label>Support e-mailadres<input type="email" value={form.support_email} onChange={(event) => setForm({ ...form, support_email: event.target.value })} placeholder="support@leverancier.nl" /></label><label>Support telefoonnummer<input value={form.support_phone} onChange={(event) => setForm({ ...form, support_phone: event.target.value })} /></label><div className="formActions full"><button className="primary">Leverancier opslaan</button></div></form>}<div className="cardGrid">{suppliers.map((supplier) => editing[supplier.id] ? <form className="entityCard supplierDetailsForm" key={supplier.id} onSubmit={(event) => saveSupplier(event, supplier.id)}><span>{supplier.active ? "Actief" : "Inactief"}</span><h3>Leveranciersgegevens</h3><label>Naam leverancier *<input required value={editing[supplier.id].name} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], name: event.target.value } }))} /></label><label>Contactpersoon<input value={editing[supplier.id].contact_name} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], contact_name: event.target.value } }))} /></label><label>Algemeen e-mailadres<input type="email" value={editing[supplier.id].email} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], email: event.target.value } }))} /></label><label>Telefoonnummer<input value={editing[supplier.id].phone} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], phone: event.target.value } }))} /></label><label>Support e-mailadres<input type="email" value={editing[supplier.id].support_email} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], support_email: event.target.value } }))} placeholder="support@leverancier.nl" /></label><label>Support telefoonnummer<input value={editing[supplier.id].support_phone} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], support_phone: event.target.value } }))} /></label><label>Notities<textarea value={editing[supplier.id].support_notes} onChange={(event) => setEditing((current) => ({ ...current, [supplier.id]: { ...current[supplier.id], support_notes: event.target.value } }))} placeholder="Openingstijden, portaal of aanvullende afspraken" /></label><div className="formActions"><button type="submit" className="primary" disabled={savingId === supplier.id}>{savingId === supplier.id ? "Opslaan…" : "Gegevens opslaan"}</button><button type="button" className="secondaryButton" onClick={() => setEditing((current) => { const next = { ...current }; delete next[supplier.id]; return next; })}>Annuleren</button></div></form> : <article className="entityCard" key={supplier.id}><span>{supplier.active ? "Actief" : "Inactief"}</span><h3>{supplier.name}</h3><strong>{products.filter((product) => product.supplier_id === supplier.id).length}</strong><small>Contactpersoon: {supplier.contact_name || "niet ingevuld"}</small><small>Support: {supplier.support_email || "geen supportadres"}</small><small>{supplier.phone || supplier.support_phone || "geen telefoonnummer"}</small>{canManage && <button type="button" className="secondaryButton" onClick={() => beginEdit(supplier)}>Gegevens invullen/bewerken</button>}</article>)}</div>{!suppliers.length && <Empty text="Geen leveranciers gevonden." />}</DataPage>;
}
  ...
  return <><section className="pageIntro"><p className="eyebrow">Horeca OS AI</p><h2>Veilige AI-assistent</h2><p>Vragen over directie, foodcost, reviews, marketing en operatie binnen de gekozen scope.</p></section>
    <section className="assistantLayout"><aside className="panel chatHistory"><button className="primary" onClick={() => { setConversationId(""); setChat([]); }}>Nieuw gesprek</button><h3>Geschiedenis</h3>{conversations.map((item) => <button className={conversationId === item.id ? "history active" : "history"} key={item.id} onClick={() => openConversation(item.id)}><b>{item.title}</b><small>{item.use_case}</small></button>)}</aside>
      <article className="panel chatPanel"><div className="useCases">{[["ceo","CEO"],["foodcost","Foodcost"],["reviews","Reviews"],["marketing","Marketing"],["operations","Operatie"]].map(([key,label]) => <button key={key} className={useCase === key ? "chip active" : "chip"} onClick={() => setUseCase(key)} disabled={Boolean(conversationId)}>{label}</button>)}</div>
        ...
        {error && <div className="notice">{error}</div>}<form action={sendMessage} className="chatComposer"><textarea name="message" maxLength="4000" required placeholder="Bijvoorbeeld: welke gerechten vragen vandaag marge-aandacht?" /><button className="primary" disabled={sending}>{sending ? "Analyseren…" : "Versturen"}</button></form>
      </article></section></>;
}
