  ...
  return (
    <div className="shell">
      ...
      <main className="content">
        ...
        {activeView === "reviews" && featureVisibility.reviews && <ReviewsInbox workspaceId={workspaceId} businessId={businessId} businesses={visibleBusinesses} canManage={isOwner || canUseFeature("reviews:manage") || canUseFeature("reviews:respond")} canAdd={isOwner || canUseFeature("reviews:manage")} />}
...
function ReviewsInbox({ workspaceId, businessId, businesses, canManage, canAdd }) {
  const [reviews, setReviews] = useState([]); const [reviewMessage, setReviewMessage] = useState(""); const [statusFilter, setStatusFilter] = useState("all"); const [reviewPage, setReviewPage] = useState(1); const [reviewPageSize, setReviewPageSize] = useState(2);
  ...
  useEffect(() => { setReviewPage(1); }, [statusFilter, businessId]);
  async function addReview(event) { event.preventDefault(); const form = new FormData(event.currentTarget); const sourceRating = String(form.get("rating")); const { error } = await supabase.from("customer_reviews").insert({ workspace_id: workspaceId, business_id: String(form.get("businessId")), source: String(form.get("source")), reviewer_name: String(form.get("reviewer") || "").trim() || null, rating: Number(sourceRating), source_rating: sourceRating, title: String(form.get("title") || "").trim() || null, review_text: String(form.get("text") || "").trim(), reviewed_at: new Date().toISOString() }); setReviewMessage(error ? `Review niet opgeslagen: ${error.message}` : "Review toegevoegd aan de inbox."); if (!error) { event.currentTarget.reset(); loadReviews(); } }
  ...
  async function uploadReviews(event) {
    ...
    const payload = rows.slice(0, 500).map((row) => ({ workspace_id: workspaceId, business_id: business, source, reviewer_name: row.name || null, rating: row.rating, source_rating: String(row.rating), title: row.title || null, review_text: row.text, reviewed_at: reviewDateIso(row.date) }));
