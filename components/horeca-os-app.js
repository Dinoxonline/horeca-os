...
function SocialInbox({ workspaceId, businessId, businesses, canManage }) {
      ...
      {visible.map((item) => {
        ...
        return <article className={`socialInboxItem ${item.direction}`} key={item.id}>
          ...
          <div className="socialInboxActions">{item.permalink && <a className="secondaryButton" href={item.permalink} target="_blank" rel="noreferrer">Openen op {channelName(item)}</a>}
            {canManage && item.content_type === "comment" && <details className="socialReply"><summary>Reageren</summary><textarea placeholder="Schrijf je reactie aan de gast" /><button type="button" className="primary" disabled>Nog niet publiceren</button><small>De reactie wordt pas geplaatst nadat Meta het publicatierecht voor Horeca OS heeft bevestigd.</small></details>}
          </div>
        </article>;
      })}
    </div>
