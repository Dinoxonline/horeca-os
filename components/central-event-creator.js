...
export default function CentralEventCreator({ workspaceId, businessId, businesses, session }) {
  ...
  const filteredEventCampaigns = useMemo(() => eventCampaigns.filter((item) => {
    ...
    const workStatus = item.workflow_status === "approved" ? "approved" : item.published_at ? "published" : item.scheduled_for ? "scheduled" : "draft";
  ...
  async function setConceptApproval(item, approved) {
    ...
    setResult(null);
    try {
      const { error } = await supabase.from("social_content_items").update({ workflow_status: approved ? "approved" : "new", media: nextMedia }).eq("id", item.id).eq("workspace_id", workspaceId);
      if (error) throw error;
