        ...
        return <article key={item.id}>
          <div>
            ...
            <div className="conceptActions">
              ...
              <button type="button" className="conceptApproveButton" disabled={conceptBusy} onClick={() => setConceptApproval(item, !approved)}>{approved ? "Terug naar concept" : "Goedkeuren"}</button>
            ...
            {approved && <div className="conceptSchedule">
              <label>Basis publicatiemoment<input type="datetime-local" value={conceptSchedule[item.id] || ""} onChange={(event) => setConceptSchedule((current) => ({ ...current, [item.id]: event.target.value }))} /></label>
              {item.scheduled_for ? <>
              ...
              </> : <button type="button" disabled={conceptBusy} onClick={() => scheduleConcept(item)}>Intern inplannen</button>}
            </div>}
          </div>
