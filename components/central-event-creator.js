...
export default function CentralEventCreator({ workspaceId, businessId, businesses, session }) {
  ...
  async function saveIncompleteDraft() {
    ...
    try {
      ...
      const channel_payloads = {
        ...
        facebook: { text: form.facebookText.trim() || form.shortDescription.trim() || form.description.trim(), cta: common.cta, image_url: imageFor("landscape", ["square"]), placements: form.facebookPlacements, group_sharing: { mode: "guided", groups: facebookGroups.filter((group) => selectedFacebookGroupIds.includes(String(group.id))).map((group) => ({ id: group.id, name: group.name, url: group.group_url })) } },
  ...
  async function createPromotionDraft(websiteEvent, calendarDelivery = null) {
    ...
    const channel_payloads = {
      ...
      facebook: { text: form.facebookText.trim() || form.shortDescription.trim(), cta: common.cta, image_url: imageFor("landscape", ["square"]), placements: form.facebookPlacements, group_sharing: { mode: "guided", groups: facebookGroups.filter((group) => selectedFacebookGroupIds.includes(String(group.id))).map((group) => ({ id: group.id, name: group.name, url: group.group_url })) } },
  ...
  return <section className="panel" style={{ marginBottom: 24 }}>
    ...
    {form.preparePromotion && <div className="channelDetails">
      ...
      </fieldset>}      {form.channels.facebook && <fieldset><legend>Facebook — klaarzetten voor publicatie</legend><div className="placementChoices"><span>Plaatsing *</span><label className="check"><input type="checkbox" checked={(form.facebookPlacements || []).includes("feed")} onChange={() => toggleFacebookPlacement("feed")} /> Facebookpagina</label></div><label>Berichttekst<textarea rows={3} value={form.facebookText} onChange={(e) => update("facebookText", e.target.value)} placeholder="Leeg = korte promotietekst" /></label><small>Horeca OS vult afbeelding, datum, locatie, tickets en Eventin-link automatisch aan. Publiceren gebeurt pas met de aparte bevestigingsknop bij het opgeslagen evenement.</small><div className="facebookGroupPicker"><strong>Groepen waarin je dit evenement wilt delen</strong>{facebookGroupsLoading && <p>Facebookgroepen laden…</p>}{facebookGroups.map((group) => <div className="facebookGroupChoice" key={group.id}><label className="check"><input type="checkbox" checked={selectedFacebookGroupIds.includes(String(group.id))} onChange={() => setSelectedFacebookGroupIds((current) => current.includes(String(group.id)) ? current.filter((id) => id !== String(group.id)) : [...current, String(group.id)])} /> {group.name}</label><button type="button" onClick={() => removeFacebookGroup(group)}>Verwijderen</button></div>)}{!facebookGroupsLoading && facebookGroups.length === 0 && <p>Nog geen groepen opgeslagen voor deze vestiging.</p>}<div className="facebookGroupAdd"><input value={newFacebookGroup.name} onChange={(event) => setNewFacebookGroup((current) => ({ ...current, name: event.target.value }))} placeholder="Naam van de groep" /><input value={newFacebookGroup.url} onChange={(event) => setNewFacebookGroup((current) => ({ ...current, url: event.target.value }))} placeholder="https://www.facebook.com/groups/…" /><button type="button" onClick={saveFacebookGroup}>Groep toevoegen</button></div>{facebookGroupsError && <p className="brevoAudienceError">{facebookGroupsError}</p>}<small>Na publicatie op je Facebookpagina opent Horeca OS iedere gekozen groep apart. De tekst wordt gekopieerd; jij bevestigt de plaatsing in Facebook.</small></div></fieldset>}
