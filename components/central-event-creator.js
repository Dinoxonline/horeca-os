    ...
    {isEvent && <div className="existingWebsiteEvents">
      ...
      {managedWebsiteEvents.length > 0 && <div className="managedEventGrid">{managedWebsiteEvents.map((eventItem) => {
        ...
        return <article key={eventItem.id}>
          ...
          {incomplete && <small>Niet alle Eventin-velden zijn beschikbaar. Controleer datum, tijd en locatie vÃ³Ã³r je dit evenement bewerkt.</small>}
          <div className="managedEventActions">{eventItem.url && <a href={eventItem.url} target="_blank" rel="noreferrer">Website openen</a>}<button type="button" disabled={linked || importingEventId === eventItem.id} onClick={() => importManagedWebsiteEvent(eventItem)}>{linked ? "Al gekoppeld" : importingEventId === eventItem.id ? "Koppelenâ€¦" : "Aan Horeca OS koppelen"}</button></div>
        </article>;
      })}</div>}
    </div>}
