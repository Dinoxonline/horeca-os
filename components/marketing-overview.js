...
function ComparisonCard({ label, item }) {
  const distribution = distributionFor(item);
  const description = distribution.common?.description || distribution.common?.short_description || item.body || "Geen omschrijving";
...
function comparisonDifference(item, matchItem) {
  if (!matchItem) return "";
  const left = distributionFor(item); const right = distributionFor(matchItem);
  const titleDifferent = eventText(item).trim() !== eventText(matchItem).trim();
  const descriptionLeft = left.common?.description || left.common?.short_description || item.body || "";
  const descriptionRight = right.common?.description || right.common?.short_description || matchItem.body || "";
  const descriptionDifferent = descriptionLeft.trim() !== descriptionRight.trim();
