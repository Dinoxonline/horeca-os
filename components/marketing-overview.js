  ...
  return <div className="marketingDifferenceActions" aria-label="Vervolgstap voor afwijkende bronnen">
    ...
    <div>{state.differences.map(source => {
      ...
      return <button type="button" className="secondaryButton" key={channel}
        ...
        {channel === "website" ? "Naar website bijwerken" : "Naar Facebook handmatig bijwerken"}
      </button>;
    })}</div>
  </div>;
}
  ...
  return <details className="marketingDetailFold marketingDuplicateReview">
    ...
    <div className="marketingDuplicateRecords">
      ...
      {others.map(candidate => <section className={`marketingDuplicateRecord ${matchItem?.id === candidate.id ? "selected" : ""}`} key={candidate.id} aria-label={`Agendapunt ${eventReference(candidate)}`}>
        ...
        <button type="button" className="secondaryButton" aria-pressed={matchItem?.id === candidate.id} disabled={busy || !onChooseMatch} onClick={() => onChooseMatch(candidate)}>{matchItem?.id === candidate.id ? "Gekozen voor vergelijking" : "Dit agendapunt vergelijken"}</button>
      </section>)}
    </div>
    {matchItem && <>
      <details key={matchItem.id} className="marketingDetailFold marketingDuplicateTexts"><summary>Teksten naast elkaar bekijken</summary>
      ...
      <section className="marketingMergeContent" aria-label="Samenvoegvoorstel">
        ...
        <p>Je bewerkt hieronder alleen het voorstel. Er wordt pas samengevoegd als je op de laatste knop klikt. De bestaande samenvoegactie kan ook gekoppelde bronnen bijwerken.</p>
        ...
        <button type="button" className="primaryButton" disabled={busy || !onMerge || !mergeTitle.trim()} onClick={() => onMerge({ title: mergeTitle, description: mergeDescription })}>{busy ? "Even geduld…" : "Deze twee agendapunten samenvoegen"}</button>
      </section>
    </>}
  </details>;
}

export function EventDetails({ workspaceId, session, onInstagramPublished, item, matchItem, sameDayItems = [], sourceComparisonItems = [], sourceComparisonCheck = "idle", business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onUpdateWebsite, facebookLinkCheck, onConfirmFacebook, onCompareSources, comparing, linking, syncError }) {
...
.marketingMergeContent>.primaryButton{justify-self:start;margin:0;white-space:normal}
