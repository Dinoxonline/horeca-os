...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      mapWithConcurrency(managedItems.slice(0, 100), 4, async (item) => {
        ...
        const run = (comparisonRuns.current[id] || 0) + 1;
        comparisonRuns.current[id] = run;
        setSourceComparisonChecks(current => ({ ...current, [id]: "pending" }));
        try {
          const comparison = await fetchSourceComparison(item);
          if (active && comparisonRuns.current[id] === run) {
            if (comparison) setSourceComparisons(current => ({ ...current, [id]: comparison }));
            setSourceComparisonChecks(current => ({ ...current, [id]: comparison ? "done" : "error" }));
          }
        } catch {
          if (active && comparisonRuns.current[id] === run) setSourceComparisonChecks(current => ({ ...current, [id]: "error" }));
        }
      });
    }
    load();
  ...
  async function compareSources(item) {
    ...
    try {
      const comparison = await fetchSourceComparison(item);
      if (comparisonRuns.current[id] !== run) return;
    ...
    } catch (compareError) { setSourceComparisonChecks(current => ({ ...current, [id]: "error" })); setError(compareError.message || "De bronnen konden niet opnieuw worden vergeleken."); }
