...
function ComparisonCard({ label, item }) {
  const distribution = distributionFor(item);
  return <div className="marketingComparisonCard"><p className="eyebrow">{label}</p><h4>{eventText(item)}</h4><dl><div><dt>Datum</dt><dd>{formatDate(itemStart(item), { weekday: "short", day: "numeric", month: "short", year: "numeric" })}</dd></div><div><dt>Locatie</dt><dd>{distribution.common?.location || "Geen locatie"}</dd></div><div><dt>Status</dt><dd>{statusFor(item, distribution).label}</dd></div></dl><p className="marketingComparisonDescription">{distribution.common?.description || distribution.common?.short_description || item.body || "Geen omschrijving"}</p></div>;
}
...
function EventDetails({ item, matchItem, sameDayItems = [], business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, linking }) {
  ...
  const matchDistribution = distributionFor(matchItem); const selectedDescription = distribution.common?.description || distribution.common?.short_description || item.body || "";
  const matchDescription = matchDistribution.common?.description || matchDistribution.common?.short_description || matchItem?.body || "";
...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  function chooseSameDateMatch(candidate) {
    ...
    if (isExternalEvent(candidate)) return linkExternalEvent(candidate, selectedItem);
    if (isExternalEvent(selectedItem)) return linkExternalEvent(selectedItem, candidate);
    return mergeManagedDuplicate(selectedItem, candidate);
  }
  ...
  return <section className="panel marketingCalendarPanel"><div className="panelHead marketingCalendarHead"><div><p className="eyebrow">MARKETINGAGENDA</p><h2>{title}</h2><p>Bekijk de planning van beide vestigingen naast elkaar. Zo zie je direct wanneer evenementen op dezelfde dag vallen.</p></div><button type="button" className="secondaryButton" onClick={() => setRefreshKey((value) => value + 1)} disabled={busy}>{busy ? "Agenda laden…" : "Agenda verversen"}</button></div>
    ...
    {selectedItem && <EventDetails item={selectedItem} matchItem={items.find((entry) => entry.id === selectedItem.potentialMatch?.id)} sameDayItems={sameDateCandidates(selectedItem)} business={businessById.get(String(selectedItem.business_id))} onClose={() => setSelectedItem(null)} onLink={() => linkExternalEvent(selectedItem)} onSyncContent={() => resyncEventContent(selectedItem)} onLinkExisting={(content) => { const matchingItem = items.find((entry) => entry.id === selectedItem.potentialMatch?.id); return selectedItem.potentialMatch?.external ? linkExternalEvent(selectedItem, matchingItem, content) : mergeManagedDuplicate(selectedItem, matchingItem, content); }} onChooseMatch={chooseSameDateMatch} linking={linkingId === String(selectedItem.id)} />}
