...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  async function resyncEventContent(item, content) {
    ...
    try {
      ...
      let websiteError;
      try { websiteUpdated = await syncExternalEventContent(nextDistribution, String(item.business_id), content); }
      ...
      if (websiteError) throw new Error("De tekst is opgeslagen in Horeca OS. " + websiteError.message + " Facebook blijft handmatig.");
