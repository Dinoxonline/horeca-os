...
function EventDetails({ item, matchItem, sameDayItems = [], sourceComparisonItems = [], business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onCompareSources, comparing, linking }) {
...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  async function resyncEventContent(item) {
    if (!item || linkingId) return;
    setLinkingId(String(item.id)); setError("");
    try {
      const distribution = distributionFor(item);
      await syncExternalEventContent(distribution, String(item.business_id), { title: eventText(item), description: distribution.common?.description || item.body || "" });
  ...
  return <section className="panel marketingCalendarPanel"><div className="panelHead marketingCalendarHead"><div><p className="eyebrow">MARKETINGAGENDA</p><h2>{title}</h2><p>Bekijk de planning van beide vestigingen naast elkaar. Zo zie je direct wanneer evenementen op dezelfde dag vallen.</p></div><button type="button" className="secondaryButton" onClick={() => setRefreshKey((value) => value + 1)} disabled={busy}>{busy ? "Agenda laden…" : "Agenda verversen"}</button></div>
    ...
    <style jsx global>{`.marketingEventDetails{grid-template-columns:minmax(0,1fr)!important}.marketingEventDetails>div:not(.marketingDetailActions):not(.marketingLinkingNotice){grid-column:1/-1}.marketingDetailActions{grid-column:1/-1;align-items:center;padding:2px 0 14px;border-bottom:1px solid #d7dfe4}.marketingDetailActions>button{min-height:42px;padding:10px 15px;white-space:nowrap;border-radius:8px}.marketingDetailActions>.marketingAutoCheck{margin-left:auto}.marketingMergeContent{flex:1 0 100%;width:100%;margin-top:4px}.marketingComparisonGrid{grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px}.marketingComparisonCard{padding:16px}.marketingComparisonTitle strong{font-size:15px;line-height:1.35}.marketingComparisonDescription{max-height:240px;font-size:14px;line-height:1.55}@media(max-width:760px){.marketingDetailActions{align-items:stretch}.marketingDetailActions>button,.marketingDetailActions>.marketingAutoCheck{width:100%;margin-left:0}.marketingComparisonGrid{grid-template-columns:1fr}}`}</style>
  </section>;
}
