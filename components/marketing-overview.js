...
const channelLabels = { website: "Website", facebook: "Facebook", instagram: "Instagram", calendar: "Agenda info@leclubbbq.nl", google: "Google", predis: "Predis", other: "Overige" };
...
export function sourceComparisonStatus(item, sources = [], check = "idle") {
  ...
  const differences = remote.map(source => ({
    ...
    descriptionDifferent: descriptionFor(source.item).trim() !== localDescription,
  })).filter(source => source.titleDifferent || source.descriptionDifferent);
  const matchingSources = remote.filter(source => eventText(source.item).trim() === localTitle && descriptionFor(source.item).trim() === localDescription).map(source => source.label);
...
function SourceComparisonDifferences({ state }) {
  ...
  return <div className="marketingSourceDifferences">
    ...
    <p>Bekijk ‘Bronnen vergelijken en tekst kiezen’ en kies zelf wat je wilt bijwerken.</p>
  </div>;
}
...
function DuplicateEventReview({ item, matchItem, candidates, onChooseMatch, onMerge, busy }) {
  ...
  }, [comparisonRequest, matchItem?.id]);
  function compareCandidate(candidate) {
    setComparisonRequest({ id: candidate.id });
    onChooseMatch(candidate);
  }
  useEffect(() => {
    setMergeTitle(eventText(item));
    setMergeDescription(descriptionFor(item));
    setTextChoice("current");
...
export function EventDetails({ workspaceId, session, onPredisSaved, onCalendarSaved, onSeriesSaved, onSeriesOpen, onInstagramPublished, item, matchItem, sameDayItems = [], sourceComparisonItems = [], sourceComparisonCheck = "idle", business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onUpdateWebsite, facebookLinkCheck, onConfirmFacebook, onCompareSources, comparing, linking, syncError, mergeNotice, contentSaveNotice }) {
  ...
  function closeDetails() { if ((!baseUnsaved && !predisUnsaved && !calendarUnsaved && !seriesUnsaved) || window.confirm("Je voorbereiding is nog niet bewaard of wordt verwerkt. Wil je toch sluiten?")) onClose(); }
  ...
  const contentDirty = baseUnsaved || Boolean(chosenContent && (chosenContent.title !== savedContent.title || chosenContent.description !== savedContent.description));
  const visibleSaveNotice = contentSaveNotice?.itemId === item.id && chosenContent
    && contentSaveNotice.title === chosenContent.title && contentSaveNotice.description === chosenContent.description
