...
export function EventDetails({ item, matchItem, sameDayItems = [], sourceComparisonItems = [], business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onUpdateWebsite, facebookLinkCheck, onConfirmFacebook, onCompareSources, comparing, linking, syncError }) {
    ...
    async function load() {
      ...
      mapWithConcurrency(managedItems.slice(0, 100), 4, async (item) => {
        ...
        try {
          ...
          if (active && comparison) setSourceComparisons(current => ({ ...current, [String(item.id)]: current[String(item.id)] || comparison }));
        } catch { /* A failed comparison must not clear a verified link or newer choice. */ }
      });
    }
    load();
    return () => { active = false; };
