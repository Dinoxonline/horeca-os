...
function EventDetails({ item, matchItem, sameDayItems = [], business, onClose, onLink, onLinkExisting, onChooseMatch, linking }) {
...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  async function mergeManagedDuplicate(item, matchingItem, content = {}) {
    ...
    try {
      ...
      if (updateError) throw updateError;
      const keptDistribution = { ...keepDistribution, common: { ...(keepDistribution.common || {}), ...(content.title ? { title: content.title } : {}), ...(content.description ? { description: content.description } : {}) } };
