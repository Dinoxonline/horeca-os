...
export function EventDetails({ item, matchItem, sameDayItems = [], sourceComparisonItems = [], business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onCompareSources, comparing, linking, syncError }) {
  ...
  const sources = sourceComparisonItems.length ? sourceComparisonItems : [{ label: "Horeca OS", item }];
...
export default function MarketingOverview({ workspaceId, businesses, session }) {
  ...
  async function fetchSourceComparison(item) {
    const distribution = distributionFor(item);
  ...
  async function syncExternalEventContent(distribution, businessId, content = {}) {
    ...
    const facebookId = String(distribution.facebook_event_delivery?.external_id || distribution.provider_delivery?.facebook?.external_id || distribution.external_ids?.facebook || "").trim();
    const requests = [];
    if (/^\d+$/.test(eventinId)) requests.push(fetch("/api/marketing/website-events/create", { method: "PATCH", headers: { Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, site: siteForBusiness(businessById.get(String(businessId))), eventId: eventinId, allowLinked: true, action: "sync-content" }) }));
  ...
  async function resyncEventContent(item, content) {
    if (!item || linkingId || contentSyncRef.current) return;
    if (!session?.access_token || !content?.title?.trim() || typeof content.description !== "string") {
    ...
    try {
      const distribution = distributionFor(item);
      const nextDistribution = { ...distribution, common: { ...(distribution.common || {}), title: content.title, description: content.description } };
      const nextMedia = (item.media || []).map((entry) => entry?.kind === "campaign_distribution" ? nextDistribution : entry);
      if (!nextMedia.some((entry) => entry?.kind === "campaign_distribution")) throw new Error("De evenementgegevens ontbreken. Open het evenement opnieuw.");
      const { data: saved, error: saveError } = await supabase.from("social_content_items")
        .update({ body: content.description, media: nextMedia })
        .eq("workspace_id", workspaceId).eq("business_id", item.business_id).eq("id", item.id)
        .select("id").single();
      if (saveError) throw saveError;
      if (!saved?.id) throw new Error("De gekozen tekst kon niet worden opgeslagen.");
      const updated = { ...item, body: content.description, media: nextMedia };
      setItems((current) => current.map((entry) => entry.id === item.id ? updated : entry));
      setSelectedItem((current) => current?.id === item.id ? { ...current, ...updated } : current);
      setSourceComparisons((current) => ({ ...current, [String(item.id)]: (current[String(item.id)] || [{ label: "Horeca OS", item }]).map((source) => source.label === "Horeca OS" ? { ...source, item: updated } : source) }));
      try {
        await syncExternalEventContent(nextDistribution, String(item.business_id), content);
      } catch (externalError) {
        throw new Error("De gekozen tekst is opgeslagen in Horeca OS, maar synchroniseren met de gekoppelde bronnen is niet volledig gelukt. Je kunt opnieuw synchroniseren. " + externalError.message);
      }
      setSelectedItem(null); setRefreshKey((value) => value + 1);
    } catch (syncError) { setError(syncError.message || "De gekozen evenementtekst kon niet worden opgeslagen."); }
  ...
  return <section className="panel marketingCalendarPanel"><div className="panelHead marketingCalendarHead"><div><p className="eyebrow">MARKETINGAGENDA</p><h2>{title}</h2><p>Bekijk de planning van beide vestigingen naast elkaar. Zo zie je direct wanneer evenementen op dezelfde dag vallen.</p></div><button type="button" className="secondaryButton" onClick={() => setRefreshKey((value) => value + 1)} disabled={busy}>{busy ? "Agenda laden…" : "Agenda verversen"}</button></div>
    ...
    {selectedItem && <EventDetails item={selectedItem} matchItem={items.find((entry) => entry.id === selectedItem.potentialMatch?.id)} sameDayItems={sameDateCandidates(selectedItem)} sourceComparisonItems={selectedItem.sourceComparisonItems || sourceComparisons[String(selectedItem.id)] || []} business={businessById.get(String(selectedItem.business_id))} onClose={() => setSelectedItem(null)} onLink={() => linkExternalEvent(selectedItem)} onCompareSources={() => compareSources(selectedItem)} onSyncContent={(content) => resyncEventContent(selectedItem, content)} syncError={error} onLinkExisting={(content) => { const matchingItem = items.find((entry) => entry.id === selectedItem.potentialMatch?.id); return selectedItem.potentialMatch?.external ? linkExternalEvent(selectedItem, matchingItem, content) : mergeManagedDuplicate(selectedItem, matchingItem, content); }} onChooseMatch={chooseSameDateMatch} comparing={linkingId === `compare:${String(selectedItem.id)}`} linking={linkingId === String(selectedItem.id)} />}
