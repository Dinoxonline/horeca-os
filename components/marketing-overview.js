...
export function EventDetails({ workspaceId, session, onPredisSaved, onCalendarSaved, onSeriesSaved, onSeriesOpen, onInstagramPublished, item, matchItem, sameDayItems = [], sourceComparisonItems = [], sourceComparisonCheck = "idle", business, onClose, onLink, onLinkExisting, onChooseMatch, onSyncContent, onUpdateWebsite, facebookLinkCheck, onConfirmFacebook, onCompareSources, comparing, linking, syncError, mergeNotice, contentSaveNotice }) {
  ...
  function closeDetails() { if ((!predisUnsaved && !calendarUnsaved && !seriesUnsaved) || window.confirm("Je voorbereiding is nog niet bewaard of wordt verwerkt. Wil je toch sluiten?")) onClose(); }
  function openSeriesOccurrence(row) { if ((!predisUnsaved && !calendarUnsaved) || window.confirm('Je kanaalvoorbereiding is nog niet bewaard. Toch een andere uitvoering openen?')) onSeriesOpen?.(row); }
