...
export default function StaffTickets({ workspaceId, canManage = false, session }) {
  ...
  const [attachmentViewState, setAttachmentViewState] = useState({});
  ...
  async function load() {
    ...
    if (error) setMessage("De tickets konden niet worden geladen. Controleer je verbinding en probeer opnieuw.");
      ...
      setNoteDrafts((current) => nextTickets.reduce((result, ticket) => ({ ...result, [ticket.id]: current[ticket.id] ?? ticket.internal_note ?? "" }), {}));
  ...
  async function saveInternalNote(ticket) {
    const note = String(noteDrafts[ticket.id] || "").trim();
    setNoteState((current) => ({ ...current, [ticket.id]: "saving" }));
    const { error } = await supabase.from("staff_tickets").update({ internal_note: note || null, updated_at: new Date().toISOString() }).eq("workspace_id", workspaceId).eq("id", ticket.id);
    if (error) {
      setNoteState((current) => ({ ...current, [ticket.id]: "error" }));
      setMessage("De interne notitie kon niet worden opgeslagen.");
      return;
    }
    setTickets((current) => current.map((item) => item.id === ticket.id ? { ...item, internal_note: note || null } : item));
    setNoteState((current) => ({ ...current, [ticket.id]: "saved" }));
  }

  async function openAttachment(ticket, attachment) {
    setAttachmentViewState((current) => ({ ...current, [attachment.path]: "opening" }));
    const response = await fetch(`/api/staff-tickets/attachment?workspaceId=${encodeURIComponent(workspaceId)}&ticketId=${encodeURIComponent(ticket.id)}&path=${encodeURIComponent(attachment.path)}`, { headers: { Authorization: `Bearer ${session?.access_token || ""}` } });
    const result = await response.json().catch(() => ({}));
    if (!response.ok || !result.url) {
      setAttachmentViewState((current) => ({ ...current, [attachment.path]: "error" }));
      setMessage("De bijlage kon niet worden geopend.");
      return;
    }
    setAttachmentViewState((current) => ({ ...current, [attachment.path]: "opened" }));
    window.open(result.url, "_blank", "noopener,noreferrer");
  }

  async function uploadAttachments(ticket) {
    const files = attachmentDrafts[ticket.id] || [];
    if (!files.length) return;
    setAttachmentState((current) => ({ ...current, [ticket.id]: "sending" }));
    for (const file of files) {
      const body = new FormData();
      body.append("workspaceId", workspaceId);
      body.append("ticketId", ticket.id);
      body.append("file", file);
      const response = await fetch("/api/staff-tickets/attachment", { method: "POST", headers: { Authorization: `Bearer ${session?.access_token || ""}` }, body });
      if (!response.ok) {
        setAttachmentState((current) => ({ ...current, [ticket.id]: "error" }));
        setMessage("Niet alle bestanden konden worden toegevoegd.");
        return;
      }
    }
    setAttachmentDrafts((current) => ({ ...current, [ticket.id]: [] }));
    setAttachmentState((current) => ({ ...current, [ticket.id]: "sent" }));
    setMessage(`Bestand${files.length === 1 ? "" : "en"} toegevoegd aan ticket #${ticket.ticket_number}.`);
    load();
  }
  ...
  return <section className="panel ticketBackoffice">
    ...
    {visible.length === 0 ? <p className="empty">Geen tickets die aan deze selectie voldoen.</p> : <div className="ticketQueue">{visible.map((ticket) => <article className={`ticketCard ${ticket.priority === "urgent" ? "urgent" : ticket.priority === "hoog" ? "high" : ""}`} key={ticket.id}>
      <div className="ticketCardMain"><div className="ticketCardTop"><span className="ticketNumber">#{ticket.ticket_number}</span><span className={`ticketPriority ${ticket.priority}`}>{priorityLabels[ticket.priority] || ticket.priority}</span><span className={`ticketStatus ${ticket.status.replaceAll(" ", "-")}`}>{statusLabels[ticket.status] || ticket.status}</span>{isOverdue(ticket) && <span className="ticketStatus overdue">Te laat</span>}<time>{formatDate(ticket.created_at)}</time></div><h3>{ticket.title}</h3><p className="ticketMeta">{ticket.category} · {ticket.location || "Locatie niet opgegeven"} · gemeld door <strong>{ticket.reporter_name}</strong></p>{!ticket.assignee_id && canManage && (() => { const roles = routingRole(ticket.category); const suggestion = members.find((member) => roles.some((role) => member.roles?.includes(role))); return suggestion ? <p className="ticketRouting">Voorgesteld: <strong>{suggestion.full_name}</strong> <button type="button" className="textButton" onClick={() => updateTicket(ticket.id, { assignee_id: suggestion.id })}>Toewijzen</button></p> : null; })()}<p className="ticketDescription">{ticket.description}</p>{ticket.attachments?.length > 0 && <div className="ticketAttachments"><strong>Bijlagen:</strong> <div className="ticketAttachmentList">{ticket.attachments.map((attachment) => <button type="button" className="textButton" key={attachment.path} onClick={() => openAttachment(ticket, attachment)} disabled={attachmentViewState[attachment.path] === "opening"}>{attachmentViewState[attachment.path] === "opening" ? "Openen…" : attachment.name}</button>)}</div>{ticket.attachments.some((attachment) => attachmentViewState[attachment.path] === "error") && <small className="muted">Openen mislukt.</small>}</div>}<p className="ticketContact">Contact: {ticket.reporter_contact || "niet opgegeven"}</p><details className="ticketConversation"><summary>Gesprek ({(messagesByTicket[ticket.id] || []).length})</summary>{(messagesByTicket[ticket.id] || []).map((item) => <div className={`ticketMessage ${item.author_role === "manager" ? "fromManager" : ""}`} key={item.id}><strong>{item.author_name}</strong><time>{formatDate(item.created_at)}</time><p>{item.body}</p></div>)}{(messagesByTicket[ticket.id] || []).length === 0 && <p className="muted">Nog geen reacties.</p>}<textarea value={replyDrafts[ticket.id] || ""} onChange={(event) => setReplyDrafts((current) => ({ ...current, [ticket.id]: event.target.value }))} placeholder="Schrijf een reactie voor de medewerker…" maxLength={5000} /><button type="button" className="secondaryButton" disabled={replyState[ticket.id] === "sending" || !String(replyDrafts[ticket.id] || "").trim()} onClick={() => sendReply(ticket)}>{replyState[ticket.id] === "sending" ? "Reactie plaatsen…" : replyState[ticket.id] === "sent" ? "Reactie geplaatst" : "Reactie versturen"}</button></details>{canManage && <><label className="ticketNote">Interne notitie<textarea value={noteDrafts[ticket.id] ?? ""} onChange={(event) => { setNoteDrafts((current) => ({ ...current, [ticket.id]: event.target.value })); setNoteState((current) => ({ ...current, [ticket.id]: "dirty" })); }} placeholder="Afspraken, opvolging of oplossing" /><button type="button" className="secondaryButton" disabled={noteState[ticket.id] === "saving" || noteState[ticket.id] !== "dirty"} onClick={() => saveInternalNote(ticket)}>{noteState[ticket.id] === "saving" ? "Notitie opslaan…" : noteState[ticket.id] === "saved" ? "Notitie opgeslagen" : "Notitie opslaan"}</button>{noteState[ticket.id] === "error" && <small className="muted">Opslaan mislukt.</small>}</label><label className="ticketNote">Bestanden toevoegen<input type="file" multiple accept="image/jpeg,image/png,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={(event) => setAttachmentDrafts((current) => ({ ...current, [ticket.id]: [...event.target.files] }))} /><small>JPEG, PNG, PDF en Word · maximaal 10 MB per bestand.</small>{(attachmentDrafts[ticket.id] || []).length > 0 && <><small>{(attachmentDrafts[ticket.id] || []).map((file) => file.name).join(", ")}</small><button type="button" className="secondaryButton" disabled={attachmentState[ticket.id] === "sending"} onClick={() => uploadAttachments(ticket)}>{attachmentState[ticket.id] === "sending" ? "Bestanden toevoegen…" : attachmentState[ticket.id] === "sent" ? "Bestanden toegevoegd" : "Bestanden toevoegen"}</button>{attachmentState[ticket.id] === "error" && <small className="muted">Toevoegen mislukt.</small>}</>}</label></>}</div>
