...
export default function EventCalendar({ item, workspaceId, session, enabled, onSaved, onUnsavedChange }) {
  ...
  async function run(action, eventId, candidateConfirmed = false) {
    ...
    try {
      ...
      setSelectedCandidateId(""); setLinkConfirmed(false);
  ...
  return <section className={styles.root} aria-label={`Agenda ${EVENT_MAILBOX}`}>
    ...
    {candidates.length > 0 && <div className={styles.notice}><strong>Mogelijke bestaande afspraken — kies de juiste</strong><p>Kies eerst een afspraak. Bevestig daarna direct onder je keuze dat deze bij het evenement hoort.</p>{candidates.map(c => <section key={c.id} aria-label={`Mogelijke afspraak: ${c.subject}`} className={styles.fields}>
      <b>{c.subject}</b><p>{c.start.replace("T", " ")} · {c.location || "Geen locatie vermeld"}</p><small>{c.match?.reason}</small>
      <button type="button" className="secondaryButton" disabled={busy} aria-pressed={selectedCandidateId === c.id} onClick={() => { setSelectedCandidateId(c.id); setLinkConfirmed(false); setDifferentAppointments(false); setConfirmed(false); }}>Deze afspraak kiezen</button>
      {selectedCandidateId === c.id && <div className={styles.notice} role="region" aria-label={`Koppeling bevestigen: ${c.subject}`}><strong>Gekozen afspraak: {c.subject}</strong><p>Alleen de koppeling wordt in Horeca OS bewaard. De titel, datum en tekst van de afspraak in Outlook blijven ongewijzigd.</p><label className={styles.check}><input type="checkbox" disabled={busy} checked={linkConfirmed} onChange={e => setLinkConfirmed(e.target.checked)} />Dit is de juiste afspraak voor dit evenement.</label><button type="button" className="secondaryButton" disabled={busy || !loaded || !linkConfirmed || Boolean(saved?.operation)} onClick={() => run("link", c.id, true)}>Deze bestaande afspraak koppelen</button></div>}
    </section>)}</div>}
